...
| 网关身份与注册、CloudLink 会话、运行清单、遥测和告警         | 已实现基于 Supabase PostgreSQL 的生产 Fleet 列表、详情、注册、严格 AetherEdge Enrollment Claim API 与控制台；活跃凭据、生产 CloudLink、历史遥测和告警仍为部分基础         |
...
| 审计查询、可恢复事件传输、网络回调、数据导出、智能体工具接口 | 已实现 Supabase ES256 JWT 身份、PostgreSQL 审计 JSON/SSE 查询、独立的 `cloud.aetheriot.dev` 运维控制台和无状态 MCP HTTP；成员配置、实时传输、多个适配器和后台任务仍待建设 |
