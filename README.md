...
| Gateway identity, enrollment, CloudLink sessions, manifests, telemetry, alarms                       | Production Fleet list/detail/registration and strict AetherEdge Enrollment Claim APIs and console are implemented over Supabase PostgreSQL; active credentials, production CloudLink, history, and alarms remain partial foundations                    |
...
| Audit query, resumable event delivery, webhooks, exports, MCP application interface                  | Supabase ES256 JWT-authenticated PostgreSQL Audit JSON/SSE, the independent `cloud.aetheriot.dev` operator console, and stateless MCP HTTP are implemented; membership provisioning, live delivery, several adapters, and workers remain planned        |
