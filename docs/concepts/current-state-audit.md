...
| Gateway registration and enrollment claim  | Gateway domain/application modules plus `adapters/fleet/memory` and `adapters/fleet/postgres`                                                                                      | Domain/application, memory conformance, PostgreSQL adapter, atomic Gateway/Audit/Outbox registration and Enrollment transitions, forced-RLS Fleet projection, strict AetherEdge Claim HTTP contract, real compiled-CLI local harness, and console pairing instructions are implemented; active credential issuance, trust-key lifecycle, CA/KMS, and production CloudLink remain planned                                                                                                      |
