...
`apps/cloudlink/src/runtime.ts` is executable, so the CloudLink composition is
no longer entirely planned. It is not, however, evidence that CloudLink Gateway
authentication exists. It runs a restricted trusted-connector mode, and the
following statements are the accurate description of it.
...
The Runtime Manifest repository is memory-only for the same composition, so a
restart empties accepted declarations and Integration uplinks fail closed until
each Gateway re-reports. That is the correct failure direction, not a defect.
