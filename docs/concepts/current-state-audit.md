...
description: Distinguish executable AetherCloud layers, Broker evidence, the Broker-ACL-only CloudLink composition, and durable PostgreSQL slices from missing production surfaces
updated: 2026-08-01
status: mixed
---
...
This audit is evidence-based as of 2026-08-01. `Implemented` below always names
...
The repository is on `feat/a0-readonly-projection`. The pre-change HEAD for the
A0 read-only projection slice is `68aef78`; the preceding deployment slice began
at `6d30ddd` and the bootstrap baseline is `d698a97`. Existing tracked and untracked
...
`pnpm check` runs 38 Node documentation and configuration contract tests and 953
Vitest behavior tests across 113 files, then TypeScript, ESLint, and Prettier.
It requires no PostgreSQL instance, edge device, or Broker.
...
| Gateway registration and enrollment claim  | Gateway domain/application modules plus `adapters/fleet/memory` and `adapters/fleet/postgres`                                                                                                                                                             | Domain/application, memory conformance, PostgreSQL adapter, atomic Gateway/Audit/Outbox registration and Enrollment transitions, forced-RLS Fleet projection with explicit connection reasons, retained latest closed-Session evidence, heartbeat freshness boundary and non-health telemetry status, strict AetherEdge Claim HTTP contract, real compiled-CLI local harness, and console pairing/Cloud-observation views are implemented; active credential issuance, trust-key lifecycle, CA/KMS, and production CloudLink remain planned                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| CloudLink session and heartbeat foundation | CloudLink domain/application modules plus `adapters/cloudlink/memory`, `adapters/cloudlink/postgres`, and `adapters/cloudlink/node-crypto`                                                                                                                | Credential-authenticated use cases, epoch fencing, cursor resume, v1alpha1 challenge issue/exact retry/atomic consume, fixed per-Gateway rate limits, Ed25519 challenge and hello transcripts, memory adapter, and transactional PostgreSQL session/challenge adapter with forced RLS, plus a multi-instance durable health lease reconciler that atomically persists suspect/heartbeat-timeout Audit and Outbox evidence through an isolated `NOBYPASSRLS` worker role, are implemented; atomic credential/key lifecycle and joint conformance remain planned                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| Experimental CloudLink MQTT ingress        | `adapters/cloudlink/mqtt`, `apps/cloudlink`, `railway.cloudlink.json`, `contracts/cloudlink/v1`, and opt-in dual Broker test                                                                                                                              | Strict alpha.3 plus reviewed v1alpha1 challenge-request decoding, MQTT.js ingress, challenge/session publication, explicit trusted-connector resolution, real Mosquitto/AWS evidence, and alpha fault matrix are implemented. A deployable read-only process now exists: `apps/cloudlink/src/server.ts` with a bounded shutdown deadline, `railway.cloudlink.json`, operator-configured Broker TLS trust, and a `memory`/`postgres` projection store whose PostgreSQL mode also makes CloudLink sessions durable through the narrower `aethercloud_cloudlink_ingress` role. Gateway-signed business uplinks deliberately fail closed because per-uplink authentication is absent; the process runs only in a restricted trusted-connector mode whose sole authenticator is the operator's Broker ACL, which this repository never configures or verifies; its stable MQTT client id permits exactly one instance and its Runtime Manifest is memory-only; production shared-Broker authentication, multi-sample mapping, durable data-loss persistence, and multi-instance ownership remain planned |
| Runtime Manifest and capability foundation | Runtime domain/application modules and `adapters/runtime/memory`                                                                                                                                                                                          | AetherEdge v1 checksum, monotonic history, report/query, memory adapter, and experimental MQTT mapping implemented; no PostgreSQL adapter exists, so the CloudLink composition loses declarations on restart and Integration uplinks fail closed until each Gateway re-reports; PostgreSQL/HTTP planned                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| Home Assistant integration projection      | AetherContracts alpha.4 candidate schemas, AetherEdge connector/durable CloudLink publisher, AetherCloud application bridge, memory/PostgreSQL Integration projection adapters, `apps/api/src/integration-routes.ts`, and `apps/web/src/integrations.tsx` | Provider-neutral multi-point topology, typed observations, Edge full resynchronization, independent durable streams, strict session replay, Runtime Manifest restoration, atomic PostgreSQL fact/inbox/receipt/history/audit/outbox/ACK persistence, and bounded catalog/by-ID application and optional MCP interfaces are implemented. The public API is no longer gated: `GET /api/v1/integrations` and `GET /api/v1/integrations/:gatewayId/:integrationId` are authenticated read-only routes under `integration.projection.read`, pinning `authority: "edge-reported-copy"` and `liveStateAuthoritative: false` as schema constants, and a console view renders them with relative freshness. Published alpha.4 consumption, Gateway-signed production composition, a PostgreSQL Runtime Manifest repository, every write or control HTTP surface, and public Agent remain gated                                                                                                                                                                                                               |
| Governed Home Assistant power control      | Integration Control domain/application, strict candidate codec, memory/PostgreSQL ledgers, Node.js Ed25519 adapter, `apps/cloudlink`, and optional `apps/mcp` adapters                                                                                    | Default-off fixed power action, explicit read-only/control enablement, current-session MQTT offer/reoffer and signed receipt ingress, transactional PostgreSQL intent/offer/receipt/audit/cursor/ACK persistence with forced RLS, trusted-governance MCP tool, and bounded projection discovery/detail resources are implemented; the deployable CloudLink ingress deliberately does not wire this path, a source-level test enforces that, and no HTTP route exposes it; production ledger composition, key lifecycle/Broker evidence, released cross-repository conformance, MCP wire root, and public Agent remain gated                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| API process                                | `apps/api`, its `fleet-routes`/`audit-routes`/`integration-routes` modules, Railway configuration, and API behavior tests                                                                                                                                 | Public health/platform, authenticated Fleet list/detail/registration/Enrollment issue/status, token-authorized strict AetherEdge Claim, diagnostic Fleet connection/telemetry projections, durable CloudLink health scheduling, Audit JSON, finite SSE, and two authenticated read-only Integration projection routes are implemented, with `app.ts` reduced to a route assembler; production verifies Supabase ES256 JWTs through JWKS, trusts only administrator-controlled scope claims, permits only the exact AetherCloud console origin through credential-free CORS, and queries PostgreSQL through constrained verified-TLS connections; membership lifecycle remains planned                                                                                                                                                                                                                                                                                                                                                                                                               |
...
reads through a constrained role and verified TLS. Fleet registration and Enrollment writes use the same
forced-RLS composition. This is not evidence of complete production migration
orchestration, other bounded-context write routes, worker
...
- production active Gateway credentials, revocation, recovery, CA/KMS, and
  durable credential activation; the Gateway SQL/API composition covers
  registration, Claim issue, fingerprint-bound consume, and non-secret status
- production Gateway authentication for the CloudLink process, inbox/outbox
  ownership, backpressure, multi-instance ingress ownership, and actual edge
  integration; a deployable read-only ingress process with a PostgreSQL session
  and projection composition is now executable, along with the
  ...
  challenge/hello handshake. Credential and public-key status
...
- production Integration Control composition, offer/acknowledgement workers,
  any Integration write or control HTTP route, MCP wire service, supported
  Broker qualification, and signing-key lifecycle; their PostgreSQL ledgers,
  bounded catalog/detail queries, optional transport-neutral MCP adapters, and
  the two authenticated read-only Integration projection HTTP routes with their
  console view are executable
...
### The CloudLink composition root is authenticated only by the Broker ACL

`apps/cloudlink/src/runtime.ts` is executable and `apps/cloudlink/src/server.ts`
with `railway.cloudlink.json` makes it a deployable process, so this limitation
is no longer theoretical. A deployable composition root is still not evidence
that CloudLink Gateway authentication exists. The process
runs a restricted trusted-connector mode, and the following statements are the
accurate description of it. Anyone deciding whether to deploy it should read
[CloudLink ingress deployment](../reference/cloudlink-ingress-deployment.md)
first.

- It enables only `aether.cloudlink.integration.v1alpha1`. The governed control
  path is unreachable by construction, enforced by a source-level test.
- It does not wire Gateway-signed challenge issue, Gateway-signed hello
  acceptance, or per-uplink Gateway signature verification, because no
  production message-origin key source exists in this repository. Active
  credential issuance, trust-key lifecycle, and CA/KMS all remain planned, so
  the per-uplink signature path cannot be composed honestly.
- A session is opened by looking up the wire-supplied Gateway ID, credential ID,
  and credential generation in operator configuration. Those three values are
  non-secret identifiers carried in the message. The configured proof never
  crosses the wire and is never persisted by AetherCloud.
- **Authentication therefore depends entirely on Broker-side per-Gateway ACLs.
  Without them there is effectively no authentication**: any publisher that can
  write `{prefix}/v1/gateways/{gatewayId}/up/...` and knows those three
  identifiers obtains a session.
- The optional ingress Broker username and password are AetherCloud's own
  subscriber login. They are not per-Gateway publisher ACLs, they do not
  implement ADR-0014 decision 5, and nothing in this repository configures,
  enforces, or verifies such ACLs. The ingress starts whether or not they exist.
- ADR-0014 decision 5 expects a reviewed trusted connector to supply verified
  publisher attestation out of band for every publish. This composition consumes
  no attestation. It assumes the Broker ACL is correct and cannot detect that it
  is not. That unconsumed attestation is the explicit gap.
- The mode is appropriate for a small deployment where the operator owns the
  Broker and can confirm publisher identity out of band. It must not be
  presented as Gateway authentication and must not be claimed to support
  multi-tenant or multi-Gateway scale.

Credential verification in this mode is tautological by construction: the
operator configuration is keyed on the very identity the wire claims, so
resolving a credential proves only that the claimed identity was configured.

### The Runtime Manifest is memory-only, including in `postgres` mode

`adapters/runtime` has no PostgreSQL adapter, so the ingress holds accepted
protocol declarations in process memory alone. A restart empties them and
Integration uplinks fail closed until each Gateway reports its manifest again.
That is the correct failure direction, not a defect.

The asymmetry is sharper in `postgres` mode, where CloudLink sessions follow the
projection store and become durable while the manifest does not. A restarted
ingress then refuses uplinks from a Gateway whose persisted session it still
accepts, so the Gateway believes its session is live and has no trigger to
re-report. Its uplinks stay refused until it reconnects for some other reason. A
PostgreSQL Runtime Manifest repository remains planned and is the fix.

### A stable MQTT client id means exactly one ingress instance

The ingress client id is stable so the transport's `clean: false` Broker session
resumes across a restart instead of being orphaned. The cost is that two
instances sharing the id evict each other from the Broker and disconnect in a
loop. A0 therefore supports exactly one ingress instance per client id, and the
Railway service must not be scaled beyond one replica. This is consistent with
multi-instance ownership being listed as planned above, but it is now a
deployment constraint rather than a future concern.

## Verification evidence

### A0 read-only projection slice, 2026-08-01

- `pnpm check`: 953 Vitest behavior tests across 113 files and 38 Node contract
  tests passed; TypeScript, ESLint, and Prettier checks passed
- `pnpm test:mqtt-integration`: passed against a real local Eclipse Mosquitto
  Broker
- `apps/cloudlink/src/runtime.ts` and `apps/api/src/integration-routes.ts`
  contain no Integration Control reference, so the governed control path is
  unwired in both the deployable ingress and the HTTP surface

These are the only checks this slice re-ran. Nothing else below was repeated,
and the following are explicitly **not** verified for this slice:

- `pnpm test:home-assistant-e2e` did not run. It requires an AetherEdge runtime
  that was unavailable, so the full Home Assistant path is unverified here and
  no claim about it may be derived from this audit.
- The `postgres` projection store has not been exercised by a running API and
  ingress pair against one database. Its role grants carry the caveat recorded
  for every grant change: `pnpm check` cannot detect a missing grant.
- This audit records no deployed evidence for a CloudLink ingress service.
  `railway.cloudlink.json` is configuration, not a running deployment.

### Completed foundation, verified through 2026-07-29
...
- `pnpm check`: 820 Vitest behavior tests and 29 Node contract tests passed;
